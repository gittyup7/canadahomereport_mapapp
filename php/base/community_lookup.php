...
try{	
	...
	$sql = 'SELECT * FROM(SELECT communities."name", ST_Within(point, communities.geom) AS within
	FROM ST_SetSRID(ST_Point(:longitude, :latitude), 4326) AS point, (SELECT "geom", "name" FROM layers.com_community_boundaries_polygon) AS communities) AS temp1
	WHERE temp1."within" = true
	LIMIT 1';
