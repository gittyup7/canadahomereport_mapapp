...
try{	
   ...
   DISTINCT ON (feature.id) feature.id, feature.geom, feature.name, feature.address_ab, feature.type, feature.elem, feature.junior_h, feature.senior_h, ST_Distance(feature.geom, location, true)  as dist
FROM ST_SetSRID( ST_Point(:longitude, :latitude), 4326) As location, layers.schools_elementary_point As feature
ORDER BY feature.id, ST_Distance(feature.geom, location) ) As query
ORDER BY query.dist LIMIT 2";
