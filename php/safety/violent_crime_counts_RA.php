...
try{	
	...
	$sql = "SELECT ST_AsGeoJSON(comm.geom, 5) As geojson, comm.name, COALESCE(stats.type::varchar(20), 'Not Available') AS type, COALESCE(stats.crimes::varchar(20), 'Not Available') AS crimes 
FROM layers.com_community_boundaries_polygon AS comm LEFT OUTER JOIN layers.danger_crime_stats AS stats 
ON (comm.name = stats.name) 
WHERE type='Violence'
";
